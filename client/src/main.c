...
int main(int argc, char const * const argv[])
{
  ...
  /* -------------------- Test execution and result readout ------------------------- */
  if (result == RESULT_OK)
  {
    /* Restart the session. */
    if (BltSessionStart() != BLT_RESULT_OK)
    {
      printf("[" OUTPUT_YELLOW "TIMEOUT" OUTPUT_RESET "]\n");
      printf("Session restarting shouldn't have failed, exit program.");
    ...
    { 
      ...
      command[0] = 0;
      result = BltSessionUserCommand(1,command,&res_len,response);
      if (result == RESULT_OK)
      {
        printf("Test run ran successfully, results:\r\n"); (void)fflush(stdout);
      }  
      do
      {
        command[0] = 1;
        ...
        res_len = 255;
        result = BltSessionUserCommand(1,command,&res_len,response);
        printf("%.254s",&response[1]);
      } while (response[0] != 0 && result != BLT_RESULT_ERROR_GENERIC);
    }
  }
...
{
  printf("--------------------------------------------------------------------------\n");
  printf("BootCommander version 1.04.05. Performs firmware updates on a micro-\n");
