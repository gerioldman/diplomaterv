/************************************************************************************//**
* \file         Demo/ARMCM4_STM32F4_Nucleo_F446RE_CubeIDE/Boot/App/hooks.c
...
blt_bool CpuUserProgramStartHook(void)
{
  ...
  // Never start the user program, as the downloaded entity is supposed to function like a shared object,
  // which will be sheduled in a separate task when a XCP command is received.
  return BLT_FALSE;
...
{
  if (BLT_TRUE != FlashVerifyChecksum()) return BLT_FALSE;
  return BLT_TRUE;
...
blt_bool NvmWriteChecksumHook(void)
{
  ...
  uint32_t data = &__io_putchar;
  if (BLT_TRUE != FlashWrite(&(descriptor->send_char),sizeof(test_ptr),&data)) return BLT_FALSE;
  if (BLT_TRUE != FlashWriteChecksum()) return BLT_FALSE;
  return BLT_TRUE;
}
